...
|---|---|---|---|
| `ISSUE_TRACKER_API_URL` | path | no (default `http://localhost:8001/api`) | base REST URL of the tracker backend |
| `ISSUE_TRACKER_API_KEY` | credential | required by every tool (validated lazily) | sent as the `X-API-Key` header; stays `${VAR}`-literal in config, injected per-launch |
...
tool runs without `ISSUE_TRACKER_API_KEY`, or the backend is unreachable, it returns a
